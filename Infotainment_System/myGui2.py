...
""))
    ...
    def update(self):
        ...
        if len(input) > 1:
                if (input[0].replace('.','',1)).isdigit() and (input[1].replace('.','',1)).isdigit():
                        self.speed = int(input[0])
                        self.distance = int(input[1])
        if self.distance < 100 and self.distance > 75:
                MainWindow.timer2.timeout.connect(ui.warning)
                if MainWindow.timer2.timerId() == -1:
                        MainWindow.timer2.start(1)
        elif self.distance < 75:
                MainWindow.timer2.timeout.connect(ui.danger)
                if MainWindow.timer2.timerId() == -1:
                        MainWindow.timer2.start(1)
        else:
        ...
        #distanceFile.close()
        self.Speed.updateValue(self.speed)
        self.Distance.setText("Distance Ahead\n"+str(self.distance)+" cm")
        ser.reset_input_buffer()
    def warning(self):
        if self.currentFont == 0:
                self.Distance.setStyleSheet("color: yellow")
                self.currentFont = 1
        else:
                self.Distance.setStyleSheet("color: white")
                self.currentFont = 0
    ...
    def danger(self):
        if self.currentFont == 0:
                self.Distance.setStyleSheet("color: red")
                self.currentFont = 1
        else:
                self.Distance.setStyleSheet("color: white")
                self.currentFont = 0
...
if __name__ == "__main__":
    ...
    MainWindow.timer2 = QTimer(MainWindow)
